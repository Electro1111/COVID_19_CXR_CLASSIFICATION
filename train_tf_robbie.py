...
#ROBBIE
from eval_robbie import eval
#ROBBIE
from data_robbie import BalanceCovidDataset
#ROBBIE
import numpy as np
...
parser.add_argument('--trainfile', default='train_COVIDx5.txt', type=str, help='Name of train file')
...
outputPath = './output/'
#ROBBIE
runID = args.name + '-lr' + str(learning_rate) +'_'+args.datadir.split('/')[-1]
...
with tf.Session() as sess:
    ...
    metrics_hist = []
    #ROBBIE
    metrics = eval(sess, graph, testfiles, os.path.join(args.datadir,'test'),
         args.in_tensorname, args.out_tensorname, args.input_size)
    #ROBBIE
    metrics_hist.append((metrics))
    #ROBBIE
    np.save(runID,metrics_hist)
